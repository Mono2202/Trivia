...
Server::Server(IDatabase* database) : 
	m_database(database), m_handlerFactory(*LoginManager::getInstance(database), database, *RoomManager::getInstance(), *StatisticsManager::getInstance(database), *GameManager::getInstance(database))
{
